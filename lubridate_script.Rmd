...
mos_counts_clean$date <- lubridate::dmy(mos_counts_clean$date)

head(mos_counts_clean, 10)
### works! our date column is now recognized as a 'date and not a 'chr'
```
...
#### now to visualize! ####
```{r}
ggplot(mos_counts_time_vis, aes(x = date, y = effort_min, color = lake)) +
  geom_line() +
  labs(title = "Effort by Date", x = "Date", y = "Effort (minutes)") +
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
- WOW WOW WOW, it's chaos!!!

-we can check out each lake in more detail with 'facet_wrap()'
```{r}
ggplot(mos_counts_time_vis, aes(x = date, y = effort_min)) +
  geom_line() +
  labs(title = "Effort by Date", x = "Date", y = "Effort (minutes)") +
  theme_minimal() +
  facet_wrap(~lake)+
